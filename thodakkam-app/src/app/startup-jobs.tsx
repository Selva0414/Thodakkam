...
export default function StartupJobs() {
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const profileRes = await fetch(`https://thodakkam-1.onrender.com/api/startup/profile/${encodeURIComponent(companyName)}`);
      if (profileRes.ok) {
  ...
  const handleUpdateStatus = async (jobId: string, newStatus: string) => {
    ...
    try {
      ...
      const res = await fetch(`https://thodakkam-1.onrender.com/api/jobs/${jobId}`, {
        method: 'PUT',
  ...
  useEffect(() => {
    if (selectedCandidate) {
      ...
      if (selectedCandidate.userId && jobToTrack?.id) {
        fetch(`https://thodakkam-1.onrender.com/api/assessment-results/${selectedCandidate.userId}/${jobToTrack.id}`)
          .then(async res => {
  ...
  const handleUpdateApplicationStatus = async (newStatus: string) => {
    ...
    try {
      const res = await fetch(`https://thodakkam-1.onrender.com/api/applications/${selectedCandidate.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
