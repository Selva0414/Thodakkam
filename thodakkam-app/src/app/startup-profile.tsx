...
export default function StartupProfile() {
  ...
  const fetchProfileData = async () => {
    ...
    try {
      // 1. Fetch from DB
      const response = await fetch(`https://thodakkam-backend-47rn.onrender.com/api/startup/profile/${encodeURIComponent(companyName)}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await fetch(`https://thodakkam-backend-47rn.onrender.com/api/startup/profile/${encodeURIComponent(companyName)}`, {
        method: 'PUT',
