...
export default function StartupEditJob() {
  ...
  const handleUpdateJob = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://thodakkam-backend.onrender.com/api/jobs/${jobId}`, {
        method: 'PUT',
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.primaryBtn} onPress={handleUpdateJob} disabled={loading}>
          {loading ? (
...
const styles = StyleSheet.create({
  ...
  primaryBtn: { 
    backgroundColor: PRIMARY, width: '100%', height: 48, borderRadius: 12, 
    justifyContent: 'center', alignItems: 'center'
  },
