...
export default function StartupNetworkModal({ visible, onClose, companyName }: StartupNetworkModalProps) {
  ...
  const fetchNetwork = async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://thodakkam-backend-47rn.onrender.com/api/startup/network/${encodeURIComponent(companyName)}`);
  ...
  const toggleFollow = async (userId: string, isCurrentlyFollowing: boolean) => {
    try {
      ...
      const res = await fetch(`https://thodakkam-backend-47rn.onrender.com/api/startup/network/${encodeURIComponent(companyName)}/${endpoint}`, {
        method: 'POST',
