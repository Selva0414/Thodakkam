  ...
  try {
    ...
    if (mcqConfig && Array.isArray(mcqConfig.questions)) {
      for (const q of mcqConfig.questions) {
        ...
        if (q.correctOption === 'B') correctOptionIndex = 1;
        if (q.correctOption === 'C') correctOptionIndex = 2;
        if (q.correctOption === 'D') correctOptionIndex = 3;
...
// Submit MCQ Result
app.post('/api/assessments/:id/submit-mcq', async (req: Request, res: Response): Promise<void> => {
  try {
    const id = req.params.id as string;
